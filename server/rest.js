...
JsonRoutes.add("get", "/fhir/Sequence/:id", function (req, res, next) {
  process.env.DEBUG && console.log('GET /fhir/Sequence/' + req.params.id);
  ...
  if (accessToken || process.env.NOAUTH) {
    ...
    if (typeof SiteStatistics === "object") {
      SiteStatistics.update({_id: "configuration"}, {$inc:{
        "Sequences.count.read": 1
      }});
    }

    var id = req.params.id;
    var appointmentData = Sequences.findOne(id);
...
JsonRoutes.add("get", "/fhir/Sequence", function (req, res, next) {
  process.env.DEBUG && console.log('GET /fhir/Sequence', req.query);
  // console.log('GET /fhir/Sequence', req.query);
  ...
  if (accessToken || process.env.NOAUTH) {
    ...
    process.env.DEBUG && console.log('Sequences.find(id)', Sequences.find(databaseQuery).fetch());

    // because we're using BaseModel and a _transform() function
    // Sequences returns an object instead of a pure JSON document
    // it stores a shadow reference of the original doc, which we're removing here
    var appointmentData = Sequences.find(databaseQuery).fetch();
